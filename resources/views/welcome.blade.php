    ...
    <div class="px-20 py-10">
        ...
        <div class="grid grid-cols-4 gap-4 mt-5">
            ...
            @php
                ...
                {
                    $discount_percentage = round((($product->price - $product->discounted_price) / $product->price) * 100);
                }
            @endphp
            <a href="{{route('viewproduct',$product->id)}}" class="bg-white shadow-md rounded-lg p-4 hover:shadow-lg hover:-translate-y-2 transition-all duration-300 relative">
                @if($discount_percentage > 0)
                <p class="absolute right-0 bg-blue-500 text-white px-2 rounded-lg">-{{$discount_percentage}}%</p>
                @endif
